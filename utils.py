...
class VIS:
    def __init__(self, save_path):
        ...
        try:
            with open(os.path.join(self.path, 'mean_iu.pkl'), 'rb') as f:
                self.mean_iu = pickle.load(f)
            with open(os.path.join(self.path, 'cls_iu.pkl'), 'rb') as f:
                self.cls_iu = pickle.load(f)

            self.score_history = np.load(os.path.join(self.path, 'score_history.npy')).item()
            print('loaded hist!')
        except FileNotFoundError:
            print('not loaded hist!')
            self.mean_iu = []
            self.cls_iu = []
            self.score_history = {}
    ...
    def save_seg(self, label_im, name, im=None, gt=None):
        seg = Image.fromarray((255*label_im).astype(np.uint8), mode='P') # must convert to int8 first
        seg.palette = copy.copy(self.palette)
        if gt is not None or im is not None:
            im = im.squeeze()
            print(im.shape, gt.shape)
            gt = Image.fromarray((255*gt).astype(np.uint8), mode='P') # must convert to int8 first]
            ...
            I = Image.new('L', (label_im.shape[1]*3, label_im.shape[0]))
            I.paste(im,(0,0))
            I.paste(gt,(256,0))
            I.paste(seg,(512,0))
        ...
        print("==> saved seg at {}".format(os.path.join(self.path, 'inferred_images', name)))
    ...
    def add_sample(self, pred, gt):
        ...
        with open(os.path.join(self.path, 'mean_iu.pkl'), 'wb') as f:
            pickle.dump(self.mean_iu, f)
        with open(os.path.join(self.path, 'cls_iu.pkl'), 'wb') as f:
            pickle.dump(self.cls_iu, f)
    ...
    def compute_scores(self, suffix=0):
        ...
        np.save(os.path.join(self.path, 'score_history'), self.score_history)
...
def mean_IU(eval_segm, gt_segm):
    ...
    cl, _ = union_classes(eval_segm, gt_segm)
    n_cl = n_cl_gt = 2  # there are always 2 classes, even if not apparent from batch
