...
from scipy.misc import imresize as scimresize
...
def load_img(path, grayscale=True, interp='bilinear', target_size=None):
  ...
  if target_size:
    if img.shape[:2] != target_size:
        if grayscale:  # mask or grayscale image. use scipy
          img = scimresize(img, target_size, interp=interp, mode='F')
        else:  # resize image. use skimage
          img = skresize(img, target_size, preserve_range=True)
  return img


def list_pictures(directory, ext='jpg|jpeg|bmp|png|tiff'):
  return [
      os.path.join(root, f)
...
class ImageDataGenerator(object):
  ...
  Arguments:
      ...
      elastic_deformation_params: [alpha, sigma]
  ...
  def __init__(self,
    ...
    self.elastic_deformation_params = elastic_deformation_params
  ...
  def random_transform(self, x, seed=None, is_mask=False):
    """Randomly augment a single image tensor.

    Arguments:
        x: 3D tensor, single image.
        seed: random seed.
        is_mask: if this is the label, maybe skip some params
    ...
    if self.elastic_deformation_params:
        # x.shape is (h, w, {1,3})
        shape = x.shape[:2]  # size of displacement field ignores num of channels
        n_channels = x.shape[2]
    
        # always taking random vars to keep randomness aligned
        rand_x = np.random.random(shape)
        rand_y = np.random.random(shape)

        dx = gaussian_filter(rand_x * 2 - 1, self.elastic_deformation_params[1], mode='constant', cval=0) * self.elastic_deformation_params[0]
        dy = gaussian_filter(rand_y * 2 - 1, self.elastic_deformation_params[1], mode='constant', cval=0) * self.elastic_deformation_params[0]
    
        # applying the same displacement to all channels with broadcasting:
        dx = dx[..., np.newaxis]
        dy = dy[..., np.newaxis]
    
        px, py, pz = np.meshgrid(np.arange(shape[0]), np.arange(shape[1]), np.arange(n_channels))
        indices = np.reshape(py+dy, (-1, 1)), np.reshape(px+dx, (-1, 1)), np.reshape(pz, (-1, 1))
  ...
  def fit(self, x, augment=False, rounds=1, seed=None):
    ...
    if augment:
      ax = np.zeros(
          tuple([rounds * x.shape[0]] + list(x.shape)[1:]), dtype=floatx())
  ...
  def _flow_index(self, n, batch_size=32, shuffle=False, seed=None):
    ...
    while 1:
      ...
      if n > current_index + batch_size:
        ...
        # self.batch_index = 0
        # zzz: to guarantee each batch has the same samples
        current_batch_size = batch_size
        current_index = n - current_batch_size
...
class NumpyArrayIterator(Iterator):
  ...
  def next(self):
    ...
    batch_x = np.zeros(
        tuple([current_batch_size] + list(self.x.shape)[1:]), dtype=floatx())
    for i, j in enumerate(index_array):
      x = self.x[j]
      x = self.image_data_generator.random_transform(x.astype(floatx()))
...
class DirectoryIterator(Iterator):
  ...
  def __init__(self,
    ...
    self.data_format = data_format
    target_size_2d = self.target_size[:2]
    if self.color_mode == 'rgb':
      if self.data_format == 'channels_last':
        self.image_shape = target_size_2d + (3,)
      else:
        self.image_shape = (3,) + target_size_2d
    else:
      if self.data_format == 'channels_last':
        self.image_shape = target_size_2d + (1,)
      else:
        self.image_shape = (1,) + target_size_2d
    ...
    self.save_to_dir = save_to_dir
    self.save_prefix = save_prefix
    self.is_mask = 'mask' in save_prefix
    self.save_format = save_format

    white_list_formats = {'png', 'jpg', 'jpeg', 'bmp', 'tiff', 'npz'}
  ...
  def next(self):
    ...
    for i, j in enumerate(index_array):
      ...
      img = load_img(
          ...
          interp='nearest' if self.is_mask else 'bilinear')
      x = img_to_array(img, data_format=self.data_format)
      # x = img[..., np.newaxis]
      # x = img
      # print(self.is_mask)
      x = self.image_data_generator.random_transform(x, is_mask=self.is_mask)
      x = self.image_data_generator.standardize(x)
      batch_x[i] = x
    # optionally save augmented images to disk for debugging purposes
    if self.save_to_dir and self.shuffle:  # save only in train mode
      for i in range(current_batch_size):
        img = array_to_img(batch_x[i], self.data_format, scale=True)
        fname = '{prefix}_{index}_{hash}.{format}'.format(
            prefix=self.save_prefix,
            ...
            hash=np.random.randint(1e4),
            format=self.save_format)
        numpngw.write_png(os.path.join(self.save_to_dir, fname), img)
    ...
    if self.class_mode == 'input':
      ...
      batch_y = np.zeros((len(batch_x), self.num_class), dtype=floatx())
...
class ClassDataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, file_name, batch_size=1024, data_split=100, start=0, end=None, root_name_x='img',
                 root_name_y='seg', is_train=True, crop_size=(256, 256), classes=None, imgen_params=None):
        "initialization"
        if imgen_params is None: imgen_params = {}
        if classes is None: classes = [[0],[1],[2],[3],[4]]

        self.classes = classes

        # rotation is done on the large image before cropping
        # self.rotation_range = imgen_params.pop('rotation_range', 0)
        self.imgen = ImageDataGenerator(**imgen_params)
        self.crop_size = np.array(crop_size)

        self.f = tables.open_file(file_name, 'r')

        self.x_all = getattr(self.f.root, root_name_x)[start:end]
        # self.x_all = getattr(self.f.root, root_name_x)
        
        labels = getattr(self.f.root, root_name_y)[start:end]
        self.y_all = np.array([i for lbl in labels for i, ls in enumerate(self.classes) if lbl in ls])
    ...
    def __getitem__(self, index):
        ...
        # for some reason reading like this is much slower when reading from file
        # batch_images = self.x_all[indices, ...]
        # if len(batch_images.shape) < 4:
        #     batch_images = batch_images[..., np.newaxis]
        ...
        for i,b in enumerate(batch_cropped):
            if np.abs(b).sum() < 1e-3 or not np.isfinite(b).all():
                print('noooooooo', index, self.is_train, i)
    ...
    #
    #         if self.is_train:
    #             img = self.imgen.random_transform(img)
    #
    #         batch_images[i] = img
    #
    #     middle = np.array(batch_images[0].shape[:2]) / 2
    ...
    #     if not np.isfinite(grades).all():
    #         print('HOLLLLYYY SHIITTTTT', index, self.is_train)
    #     batch_y = keras.utils.to_categorical([i for gr in grades for i,ls in enumerate(self.classes) if gr in ls], len(self.classes))
    #
    #     return batch_cropped, batch_y
